...
class ReplayBuffer:
    ...
    def __init__(self, obs_dim, act_dim, size, device): #gid added device
        self.device = device
...
def ddpg(env_fn, actor_critic=core.MLPActorCritic, device=torch.device("cpu"), ac_kwargs=dict(), seed=0, 
    ...
    for t in range(total_steps):
        ...
        if (t+1) % steps_per_epoch == 0:
            ...
            if (epoch % save_freq == 0) or (epoch == epochs):
                logger.save_state({'env': env}, t) #gideon changed for logging all
